...
# List of disks and their corresponding new serial numbers
disk_changes = [
    ("C:", ""),  # Placeholder for random serial
    ("D:", ""),  # Placeholder for random serial
...
def logo():
    ...
    print(logoFade)
    print("Free woofer v1".center(65, "="))
    print(f"{BLUE}drexware{YELLOW} temp - Made By Drexxy - drexware.gg{RESET}".center(65))
...
def getVersion():
        ...
        response = requests.get("https://raw.githubusercontent.com/DrexWare/DrexWare_Loader/refs/heads/main/version.txt")
        response.raise_for_status()
        text = response.text
        return text
...
def main():
    ...
    map_file_path = download_file("https://github.com/DrexWare/skyware-self-leak/releases/download/af/kdmapper.exe", _mapper)
    if map_file_path:
        log("Successfully initialized 1/2 files")
    else:
        raise Exception("Make sure antivirus is off and you are connected to the internet")

    driver_file_path = download_file("https://github.com/DrexWare/skyware-self-leak/releases/download/af/s.sys", _driver)
    if driver_file_path:
